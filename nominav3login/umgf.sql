...
CREATE TABLE IF NOT EXISTS empleados (
  ...
  dep_cod VARCHAR(5) NOT NULL,
  pues_codi VARCHAR(5) NOT NULL,
  emp_sueldonom double(4,2),
  emp_estatus boolean,
  PRIMARY KEY (emp_codigo),
  CONSTRAINT fk_empleados FOREIGN KEY(dep_cod) REFERENCES departamentos(dep_codigo),
  CONSTRAINT fkpuesto FOREIGN KEY(pues_codi) REFERENCES puesto(pues_codigo)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
DROP TABLE IF EXISTS nominaencabezado;
CREATE TABLE IF NOT EXISTS nominaencabezado (
  nomi_codigo VARCHAR(5) NOT NULL,
  nomi_fecha date DEFAULT NULL,
  nomi_monto int(11) DEFAULT NULL,
  ban_codigo VARCHAR(5) DEFAULT NULL,
...
pues_codigo VARCHAR(5) NOT NULL,
